...
class FileInfo():
    def __init__(self, hash, domain, relative_path, properties, flags, virtual=False):
        self.hash = hash
        self.domain = domain
    ...
    def get_path(self, root):
        return os.path.join(root, self.hash[:2], self.hash)
...
class BackupFS(Operations):
    ...
    def _get_file_info(self, partial):
        ...
        if partial.endswith(os.sep):
            partial = partial[:-1]
        if len(partial) == 0:
            return FileInfo(None, "", None, None, 2, virtual=True)
        ...
        if isinstance(domain_tree, dict):
            if len(parts) == 1:
                return FileInfo(None, domain_path, None, None, 2, virtual=True)
            try:
        ...
        plist = biplist.readPlistFromString(row[1])
        properties = plist['$objects'][plist['$top']['root'].integer]
        return FileInfo(row[0], domain_path, relative_path, properties, row[2])
    ...
    def open(self, path, flags):
        ...
        return os.open(file_info.get_path(self.root), flags)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: mount_ios_backup.py <backup> <mountpoint>")
        sys.exit(1)
